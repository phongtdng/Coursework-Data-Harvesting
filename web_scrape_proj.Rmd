...
scrape_all <- function(category) {
  cat <- category_url[str_detect(category_url, category)]
  ...
  product_info_paths <- read_html(remDr$getPageSource()[[1]]) %>% 
    xml_find_all("//div[contains(@class, 'box_product-info')]")

  df <- map_dfr(product_info_paths, get_product_info) %>% 
