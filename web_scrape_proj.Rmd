...
```{r}
#helper function to id gender & kid
gender_id <- function(str){
  male <- c("men", "man", "hombre", "hombres", "him", "homme", "male",  "males") 

  female <- c("women", "woman", "mujer", "mujeres", "her", "femme", "female", "females", "venus") 

  kids <- c("kid", "kids", "niños", "niño", "niñas", "niña")
  
  str <- tolower(str)
  
  if(str_detect(str, paste0("\\b(", male, ")\\b", collapse = "|"))) {
    return("Male")
  } else if(str_detect(str, paste0("\\b(", female, ")\\b", collapse = "|"))) {
    return("Female")
  } else if(str_detect(str, paste0("\\b(", kids, ")\\b", collapse = "|"))) {
    return("Kid")
  } else {return(NA)}
...
cleaned_clarel <- all_products_clarel %>% 
  ...
  mutate(avg_price = round(avg_price, 2),
         gender = ifelse(category == "Hombre", "Male",mapply(gender_id, product_name))
         ) %>% 
  #take out products for kids
  filter(gender != "Kid") %>% 
