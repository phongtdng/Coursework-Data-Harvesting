...
# write_csv(hombre_products, "datasets\\hombre.csv")
```

#### Higiene page

```{r}
clarel_higiene <- category_url[str_detect(category_url,"higiene")]

remDr$navigate(clarel_higiene)
...
higiene_products <- map_dfr(product_info_paths, get_product_info)

higiene_products
```

```{r}
# #Save into a csv file to avoid scrape every time
# write_csv(higiene_products, "datasets\\higiene.csv")
```

#### Cabello page

```{r}
clarel_cabello <- category_url[str_detect(category_url,"cabello")]

remDr$navigate(clarel_cabello)
...
cabello_products <- map_dfr(product_info_paths, get_product_info)

cabello_products
```

```{r}
# write_csv(higiene_products, "datasets\\cabello.csv")
```
