...
First, the user agent is being set to provide the website that is being scraped with the information of the scraper. This is done to allow the webmaster to contact the scraper in case she is causing any problems for the functioning of the website. 
...
remDr <- rsDriver(port = 4466L, browser = "firefox",  verbose=F, chromever = NULL)
...
t_test_fun <- function(cat_f, quan_f) {
  ...
  tryCatch({
    t_test_result <- tidy(t.test(prices_male, prices_non_male, alternative = "less"))
  }, error = function(e) {NA
...
t_test_fun <- function(cat_f, quan_f) {
  cat_d <- df_balanced |> 
    filter(subcat == cat_f & quantity_unit == quan_f)
...
cat_loop <- as.list(cat_loop$subcat)
cat_loop <- unlist(cat_loop)
cat_f <- rep(cat_loop, each = 2) #change


quan_f <- c("ml", "ud")
quan_f <- rep(quan_f, times = 6)#change
...
result_cat$sub_category <- cat_f
result_cat$quantity_unit <- quan_f

result_cat<-result_cat |> 
  select(sub_category, quantity_unit, everything()) |> 
   rename(diff_male_non_male = estimate, 
         male = estimate1, 
         non_male = estimate2,
         t_statistic = statistic,
